...
public class EmpDao {
	...
	public int updateMyEmp(Emp emp) {
		String sql = """
				UPDATE emp SET email = ?, tel = ?, mobile = ?, mobile2 = ?, address = ?, married = ? WHERE emp_id = ? """;
		try (Connection conn = ds.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
	...
	private static Emp makeEmp(ResultSet rs) throws SQLException {
		...
		return new Emp(rs.getString("emp_id"), rs.getString("password"), rs.getString("ename"), rs.getLong("dept_id"),
				rs.getLong("job_id"), rs.getString("gender"), rs.getString("email"), rs.getString("tel"),
				rs.getString("mobile"), rs.getString("mobile2"), rs.getString("img_path"), rs.getString("img_original"),
				rs.getString("img_uuid"), rs.getString("img_type"), rs.getDate("hiredate").toLocalDate(),
				rs.getString("hiretype"), rs.getDate("birthday").toLocalDate(), rs.getString("birthday_type"),
				rs.getString("school"), rs.getString("major") == null ? null : rs.getString("major"),
				rs.getString("bank"), rs.getString("account"), rs.getString("address"), rs.getInt("married") == 1,
				rs.getInt("child") == 1, rs.getString("etype"), rs.getInt("vacation_count"), rs.getBoolean("hired"));
	}
}