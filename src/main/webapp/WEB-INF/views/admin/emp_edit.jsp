    ...
    <div class="content">
        <form id="emp-form" enctype="multipart/form-data">
            <table class="table">
                    ...
                    <th rowspan="4" id="photo">
                        <span id="showImage">
                            <c:if test="${empty emp.imgOriginal}">
                                <c:set var="src" value="${pageContext.request.contextPath}/image/profile_navy.png"/>
                            </c:if>
                            <c:if test="${!empty emp.imgOriginal}">
                                <c:set var="src"
                ...
                <tr>
                    <td><input type="text" value="${emp.ename}" name="ename" data-name="이름" required></td>
                    <td>
                        <select name="gender">
                ...
                <tr>
                    ...
                    <td><input type="text" name="mobile" value="${emp.mobile}" data-name="휴대전화" required></td>
                </tr>
            </table>
            ...
            <table>
                <tr>
                    ...
                    <td rowspan="2"><input type="text" name="bank" value="${emp.bank}" placeholder="은행명을 입력하세요"
                                           data-name="은행" required>
                        <input type="text" name="account" value="${emp.account}" placeholder="계좌번호를 입력하세요"
                               data-name="계좌번호" required></td>
                    <th>긴급연락처</th>
                    <td><input type="text" name="mobile2" value="${emp.mobile2}" data-name="비상연락처" required></td>
                </tr>
                <tr>
                    <th>채용구분</th>
                ...
                <tr>
                    <th>생년월일</th>
                    <td>
                        <input type="text" name="birthday" placeholder="YYYY/MM/DD" value="${emp.birthday}"
                    ...
                    <th>주소</th>
                    <td><input type="text" name="address" value="${emp.address}" data-name="주소" required></td>
                    <th>자격증</th>
                        ...
                        <c:forEach var="cert" items="${certList}">
                            <div class="cert-item">
                                <input type="text" name="cert" value="${cert.certName}"
                                       class="form-control cert-input" data-name="자격증" required>
                ...
                <tr>
                    <th>최종학력</th>
                    <td><input type="text" name="school" value="${emp.school}" data-name="최종학력" required></td>
                    <th>결혼여부</th>
                    <td>
                        <select name="married">
                        ...
                        <c:forEach var="lang" items="${langList}">
                            <div class="lang-item">
                                <input type="text" name="lang" value="${lang.langName}"
                                       class="form-control lang-input" data-name="외국어" required>
                ...
                <tr>
                    <th>전공</th>
                    <td><input type="text" name="major" value="${emp.major}"></td>
                    <th>자녀</th>
...
<script>
    $(document).ready(function () {
        ...
        $('#submit-button').on('click', function (event) {
            ...
            // AJAX 요청
            $.ajax({
                url: "${pageContext.request.contextPath}/admin/emp-manage/edit-process",
                type: "POST",
        ...
        $('#certifications').on('click', '.add-cert', function () {
            const newCert = `
				<div class="cert-item">
					<input type="text" name="cert" class="form-control cert-input" placeholder="자격증 이름 입력" data-name="자격증" required>
					<button type="button" class="btn btn-sm btn-light remove-cert">-</button>
				</div>`;
        ...
        $('#languages').on('click', '.add-lang', function () {
            const newLang = `
				<div class="lang-item">
					<input type="text" name="lang" class="form-control lang-input" placeholder="언어 이름 입력"  data-name="외국어" required>
					<button type="button" class="btn btn-sm btn-light remove-lang">-</button>
				</div>`;
